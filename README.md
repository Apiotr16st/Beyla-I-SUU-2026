...
- deployment of the observability stack consisting of Grafana Beyla and Prometheus,
...
Instead, data generation was performed dynamically during the demonstration process. The Online Boutique application generated operational telemetry through interactions initiated by the AI model. Various prompts were used to trigger application actions such as workload generation, deployment scaling, and service management.
...
The demonstration scenario was designed to validate the interaction between the AI-driven management layer and the observability stack while monitoring the resulting effects on application behavior.

Initially, the complete Kubernetes environment was deployed according to the installation procedure described in previous sections. The deployment process included the Online Boutique application, Grafana Beyla, Prometheus, Grafana, the MCP Server, and the prompt-service.

After all services became operational, the telemetry pipeline was verified to ensure that metrics generated by Grafana Beyla were correctly collected and forwarded to Prometheus.

The prompt-service was then started and connected to the MCP Server using the Server-Sent Events (SSE) endpoint. This allowed the Large Language Model to access Kubernetes management tools exposed through MCP.

Several operational scenarios were executed during the demonstration in order to generate changes in application behavior and evaluate how these changes were reflected in collected metrics.

### Prompt 1 – List application resources

**Prompt:**

> List all deployments currently running in the application namespace.

**Purpose:**

- verify communication between LLM and MCP tools,
- retrieve Kubernetes deployment information.

---

### Prompt 2 – Increase application load

**Prompt:**

> Increase the load generator to 100 users.

**Purpose:**

- generate increased HTTP/gRPC traffic,
- observe changes in request rates and response times.

---

### Prompt 3 – Scale application service
...
- observe changes in traffic distribution and performance.

---

### Prompt 4 – Restart a service

**Prompt:**

> Restart the product catalog deployment.

**Purpose:**

- simulate temporary service interruption,
- analyze effects on system metrics.

---

### Prompt 5 – Reduce generated workload

**Prompt:**

> Reduce the load generator to 10 users.

**Purpose:**

- restore application to baseline conditions,
- observe stabilization of telemetry values.

---

## 9b. Results presentation

The performed demonstration successfully showed the interaction between the AI management layer and the observability system.

During workload modifications initiated through AI-generated actions, Grafana Beyla automatically detected changes in application behavior and exported telemetry data to Prometheus without requiring any source code modifications.

The collected metrics demonstrated visible changes corresponding directly to the actions executed by the AI model.

The most significant observations included:

- increased request rates after workload generation,
- temporary response time changes during scaling operations,
- short-duration error spikes during deployment restarts,
- stabilization of metrics after reducing generated traffic.

These observations confirm that AI-generated operational actions can be directly be correlated with system behavior through automatically collected telemetry.
...
Additionally, integrating an AI model through LangChain and the MCP Server enabled automated management tasks such as deployment scaling, workload generation, and service manipulation.
