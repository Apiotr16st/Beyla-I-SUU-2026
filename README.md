...
Before executing the demonstration scenario, all required infrastructure and application components were configured and deployed within the Kubernetes environment.

The configuration process included:

- creation of the k3d Kubernetes cluster,
- deployment of the Online Boutique application in the `app` namespace,
- deployment of the observability stack consisting of Beyla, Prometheus, and Grafana,
- deployment and configuration of Grafana dashboards and data sources,
- deployment of the MCP Server with Kubernetes management tools,
- configuration of the prompt-service together with access credentials for the Gemini API.

After deployment, communication between all components was verified to ensure proper operation of the complete architecture.

---

## 8.2 Data preparation

Since the project focuses on application observability and AI-driven management rather than static datasets, no external data preprocessing stage was required.

Instead, data generation was performed dynamically during the demonstration process. The Online Boutique application generated operational telemetry through interactions initiated by the AI model. Various prompts were used to trigger application management actions and generate observable changes in system behavior.

These interactions produced runtime metrics including:

- request rates,
- response durations,
- error information,

which were automatically collected by Grafana Beyla and forwarded to Prometheus for further visualization and analysis.
...
The demonstration was performed on a local Kubernetes cluster created with K3d. The Online Boutique microservices application was deployed in the `app` namespace, while observability components were deployed in dedicated namespaces. Grafana dashboards were used to visualize telemetry data collected by Beyla and stored in Prometheus.

The MCP server was deployed as a Kubernetes service and exposed through a NodePort. A Prompt Service based on Google Gemini was executed locally and connected to the MCP server. This configuration enabled natural language prompts to trigger management actions on the Kubernetes cluster.

The following prompts were used during the demonstration.

### Prompt 1 – Querying the Kubernetes Environment

The first prompt was used to verify communication between the AI assistant and the Kubernetes cluster through the MCP interface.

```text
List all deployments currently running in the app namespace.
```

The AI assistant responded:

```text
Deployments in the app namespace: adservice, cartservice, checkoutservice, consumer, currencyservice, emailservice, frontend, loadgenerator, paymentservice, producer, productcatalogservice, recommendationservice, redis-cart, shippingservice.
```

This confirmed that the AI model could successfully access cluster information through the MCP server.

### Prompt 2 – Increasing Application Load

The second prompt was used to increase application traffic by modifying the load generator deployment.

```text
Set the load generator to 100 users and rate 5.
```

The AI assistant responded:

```text
Load generator set to 100 users and rate 5 in namespace "app".
```

The MCP server updated the Kubernetes deployment configuration, increasing the amount of generated traffic.

### Prompt 3 – Restoring Baseline Configuration

The final prompt was used to restore the original load generator configuration.

```text
Set the load generator to 10 users and rate 1.
```

The AI assistant responded:

```text
Load generator set to 10 users and rate 1.
```

The deployment was updated again and the application traffic returned to its baseline level.

## 9.2 Results Presentation

Figure 9.1 presents the Grafana dashboard in the baseline state. The dashboard displays RED metrics (Rate, Errors, Duration) collected by Beyla from the Online Boutique microservices application. Multiple services such as `adservice`, `cartservice`, and `checkoutservice` can be monitored simultaneously.

![Baseline monitoring dashboard](images/grafana_baseline.png)

**Figure 9.1.** Beyla RED Metrics dashboard showing baseline application behavior before any AI-driven management actions.

Figure 9.2 shows the system after the AI assistant received the prompt requesting an increase of the load generator configuration. The request rate significantly increased across multiple services, demonstrating that the AI assistant successfully executed a Kubernetes management action through the MCP interface.

```text
Set the load generator to 100 users and rate 5.
```

![AI-driven load increase](images/grafana_ai_increase_load.png)

**Figure 9.2.** Grafana dashboard after the AI assistant increased the load generator configuration to 100 users and rate 5 using a natural language prompt.

Figure 9.3 presents the dashboard after the AI assistant restored the original load generator configuration. The request rate returned to values close to the initial baseline, confirming successful execution of the recovery action through the MCP-enabled management workflow.

```text
Set the load generator to 10 users and rate 1.
```

![AI-driven load restoration](images/grafana_ai_restore_baseline.png)

**Figure 9.3.** Grafana dashboard after the AI assistant restored the load generator configuration to 10 users and rate 1 using a natural language prompt.

The results confirm that the proposed solution enables natural language interaction with Kubernetes resources through an MCP-based architecture. The AI assistant was able to interpret user requests, invoke the appropriate MCP tools, modify Kubernetes deployments, and produce observable changes in application behavior that were immediately reflected in Grafana dashboards.

---

# 10. Summary – conclusions

This project demonstrated an approach to modern application observability using eBPF technology combined with AI-driven operational management.

The implemented architecture successfully integrated Kubernetes, Grafana Beyla, Prometheus, Grafana, Model Context Protocol (MCP), and Large Language Models into a unified system capable of autonomous management and monitoring.

The performed experiments confirmed that Grafana Beyla can collect useful telemetry information without requiring any source code modifications or manual instrumentation. Compared to traditional monitoring approaches, this significantly reduces operational complexity.

Additionally, integrating an AI model through LangChain and the MCP Server enabled automated management tasks through natural language interaction. The demonstrated scenarios included querying Kubernetes resources and dynamically modifying application workload parameters, with the resulting effects immediately observable through the telemetry pipeline.

The obtained results indicate that the combination of AI systems and kernel-level observability technologies can provide a promising foundation for future intelligent operational environments.

Potential future improvements may include:

- implementation of automated anomaly detection,
- integration with alerting systems,
- support for advanced AI decision-making mechanisms,
- extension of telemetry collection with logs and detailed distributed tracing.

Overall, the proposed solution demonstrates the feasibility and effectiveness of combining autonomous AI systems with modern observability techniques.

---

# 11. References

[1] Grafana Labs, *Grafana Beyla Documentation*  
Available: https://grafana.com/docs/beyla/

[2] Grafana Labs, *Grafana Documentation*  
Available: https://grafana.com/docs/grafana/

[3] Prometheus Authors, *Prometheus Documentation*  
Available: https://prometheus.io/docs/

[4] Linux Foundation, *eBPF Documentation*  
Available: https://ebpf.io/

[5] Kubernetes Authors, *Kubernetes Documentation*  
Available: https://kubernetes.io/docs/

[6] Google Cloud, *Online Boutique Microservices Demo Application*  
Available: https://github.com/GoogleCloudPlatform/microservices-demo

[7] LangChain, *LangChain Documentation*  
Available: https://docs.langchain.com/

[8] Model Context Protocol, *MCP Documentation*  
Available: https://modelcontextprotocol.io/

[9] Google AI, *Gemini API Documentation*  
